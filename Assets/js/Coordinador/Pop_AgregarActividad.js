$(document).ready(function () {
    ...
    $("#btnAdd_A").click(function () {
        ...
        if (!checkbox.checked) {
            ...
            var NEncAct = $("#Tabla_Int tr").length;

            var IdsEncAct = new Array();

            for (var j = 0; j <= NEncAct - 1; j++) {
                IdsEncAct[j] = $("#Tabla_int tr").eq(j).children("td").eq(0).find("input:hidden").val();
            }
        ...
        $.ajax({
            ...
            type: "POST",
            data: { NombreAct: NombreAct, DescripcionA: valorDC, FechaI: FechaI, FechaF: FechaF, Requerimientos: ReqAct, NRequerimientos: ReqActCont, EncargadosA: EncActividad, NEncargados: NEncAct, ComisionAA: ComApoyo, NComisiones: NComApoyo, PersonalAA: PApoyo, NPersonalA: NOtrosP, IDComisionAsig: IDComisionAsig },
            cache: false,
            success: function (result) {
                console.log(result);
                if (result.length == 1) {
